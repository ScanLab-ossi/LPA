...
def create_vector(df):
    ...
    sums = (
        df.groupby("category", as_index=False)["element"]
        .sum()
        .rename(columns={"element": "total_weight"})
    )

    vector = pd.merge(counts, sums, on="category")
    vector["local_weight"] = vector["frequency_in_category"] / vector["total_weight"]
    vector = vector.sort_values(["category", "local_weight"], ascending=[True, False])

    lrnk = vector.groupby("category").rank(method="first", ascending=False)[
        "local_weight"
    ]
    lrnk.name = "lrnk"
...
def create_signatures(df, epsilon_frac=2, sig_length=500):
    ...
    num_of_elements = vector.iloc[:, [1, -1]].groupby("category").max()
    num_of_elements["beta"] = 1 - ((len(dvr) - num_of_elements["lrnk"]) * epsilon)
    vecs = vector.pivot_table(
